...
Lemma star_CS_SeqStep:
  forall c2 st c st' c' l,
    star cstep (c, st) l (c', st') ->
    star cstep ((c;;c2), st) l ((c';;c2), st').
Proof.
...
Theorem ceval_to_csteps:
  forall c st st' l,
  c / st \\ st' --> l -> star cstep (c, st) l (SKIP, st').
Proof.
  induction 1.
- (* SKIP *)
  apply star_refl.
...
- (* sequence *)
  eapply starans. apply star_CS_SeqStep. apply IHceval1.
  replace l' with ([] ++ l' ).
...
- (* output *)
  apply star_one. apply CS_Output. auto.
Qed.
...
Theorem csteps_to_ceval:
  forall st c st' l,
  star cstep (c, st) l (SKIP, st') -> c / st \\ st' --> l.
Proof.
  intros. dependent induction H.
- apply E_Skip.
...
Definition kterminates (c: com) (st: state) (st': state) l : Prop :=
  star kstep (c, Kstop, st) l (SKIP, Kstop, st').

Definition kdiverges (c: com) (st: state) s : Prop :=
  infseq kstep s (c, Kstop, st) .

Definition kreacts (c: com) (st: state) s : Prop :=
  infseq_prod kstep s (c, Kstop, st).

Definition ksilent (c : com) (st : state) (st' : state) l : Prop :=
  exists c' k', star kstep (c, Kstop, st) l (c', k', st') /\
...
Theorem ceval_to_ksteps:
  forall c st st' l,
  c / st \\ st' --> l ->
  forall k, star kstep (c, k, st) l (SKIP, k, st').
Proof.
  induction 1; intros.
- (* SKIP *)
  apply star_refl.
...
Lemma ksteps_append_ckeval:
  ...
  star kstep (c1, k1, st1) l (c2, k2, st2) ->
  ckeval c2 k2 st2 st3 l' ->
  ckeval c1 k1 st1 st3 (l ++ l').
Proof.
  intros. dependent induction H.
