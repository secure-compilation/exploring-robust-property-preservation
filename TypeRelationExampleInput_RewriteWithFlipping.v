...
Definition S := lts SExp SInput unit SInput SRetState STrans SReturn_stops.
Definition SStuck (se : S) := stuck se.
...
Lemma nonstck : forall (se : S), ~ stuck se -> (exists sv, SRetState se sv) \/ (exists sl se', STrans sl se se').
intros se nstck.
...
Definition T := lts TExp TInput unit TInput TRetState TTrans TReturn_stops.


Definition TStuck (te : T) := stuck te.


Lemma determinate_inputs_target : determinate_inputs T.
Admitted.

Lemma determinate_target : determinate T.
Admitted.
...
Inductive rel_inp : inputs S -> inputs T -> Prop := 
...
Lemma is_input_total : partial_rel_input_total S T rel_inp.
intros se se' si0 ti H.
induction H.
Admitted.

Inductive simulation0 : S -> T -> Prop :=
...
Definition sim_typed : S -> T -> Prop := fun se te => exists ty, typing se ty /\ simulation0 se te.
...
Lemma simulation_allpositive : forall (se se' : S) (te te' : T) n, n > 0 -> simulation0 se te -> STrans (inp (SIBoo true)) se se' -> TTrans (inp(TINat n)) te te' -> simulation0 se' te'.
